...
export default function PYQPage() {
  ...
  const handleAnalyze = async () => {
    if (!file || !user) { toast.error("Please select a file"); return; }
    setLoading(true);
    ...
    try {
      ...
      await saveUpload(user.uid, { type: "pyq", fileName: file.name, fileUrl, subject: subject || "General", analysis: result as unknown as Record<string, unknown> });
      await savePrediction(user.uid, { type: "pyq", subject, ...result });
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="mb-4">
          ...
          <input type="text" placeholder="e.g., Operating Systems, Mathematics"
        ...
        <div onClick={() => fileRef.current?.click()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { setFile(f); setAnalysis(null); } }}
          onDragOver={(e) => e.preventDefault()}
          className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${file ? "border-blue-500 bg-blue-50/50 dark:bg-blue-950/20" : "border-border hover:border-blue-400 hover:bg-muted/30"}`}>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">
                {uploadProgress < 35 ? "Reading paper..." : uploadProgress < 60 ? "Analyzing patterns..." : uploadProgress < 90 ? "Generating predictions..." : "Done!"}
              </span>
        ...
        <Button onClick={handleAnalyze} disabled={!file || loading}
          ...
          {loading ? (
            ...
            <div className="flex items-center gap-2"><Sparkles className="w-4 h-4" />Analyze & Predict Questions</div>
          )}
        </Button>
      </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {analysis.trends.map((trend, i) => (
                  <Badge key={i} className="bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300 border-blue-200 text-xs">{trend}</Badge>
                ))}
              </div>
            </div>

            {/* Charts + Repeated Questions */}
                  ...
                  {analysis.repeatedQuestions.map((q, i) => (
                    <div key={i} className="border border-border rounded-xl p-3">
                      ...
                      <div className="progress-bar mt-1.5">
                        <div className="progress-fill" style={{ width: `${q.probability}%`, background: q.probability >= 80 ? "#22c55e" : q.probability >= 60 ? "#f59e0b" : "#ef4444" }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                {analysis.predictions.map((pred, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <div className="progress-bar mt-2 ml-8">
                      <div className="progress-fill" style={{ width: `${pred.probability}%`, background: pred.probability >= 80 ? "#22c55e" : pred.probability >= 60 ? "#f59e0b" : "#ef4444" }} />
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-400">
                These predictions are based on historical patterns. Upload more years of PYQ papers for higher accuracy. Always cover the full syllabus — use these predictions as a guide, not a guarantee.
              </p>
            </div>
          </motion.div>
        )}
