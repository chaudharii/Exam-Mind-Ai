...
export default function BillingPage() {
  ...
  const handleCancel = async () => {
    // Payments disabled — inform user and noop
    try {
      setCancelling(true);
      toast.error("Payments are disabled — no subscription to cancel.");
    } finally {
      setCancelling(false);
    }
  };

  // Payments have been disabled — site is now free.
        ...
        <div className="grid md:grid-cols-1 gap-4">
          <div className="bg-card border-2 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h4 className="font-semibold">All features are free</h4>
                <p className="text-sm text-muted-foreground">ExamMind AI is now free for all users — no payments required.</p>
              </div>
              <Badge className="bg-green-100 text-green-700 text-xs">Free</Badge>
            </div>
            <div className="text-sm text-muted-foreground">You have access to all tools and features without subscribing.</div>
          </div>
        </div>
      </motion.div>

      {/* Payments removed — site is free */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        className="bg-card border border-border rounded-2xl p-6">
        <h3 className="font-semibold mb-2">Payments Disabled</h3>
        <p className="text-sm text-muted-foreground">Payment options have been removed. All features are available for free.</p>
      </motion.div>
